#ifndef MULTIGRID_INIT_H
#define MULTIGRID_INIT_H
...
public:
    ...
    MultigridSimulation<T, dim>& sim; // This contains the real scene data.
    ...
    MultigridInitBase(MultigridSimulation<T, dim>& sim, const int test_number)
        : sim(sim)
        , scene(sim.scene)
        , init_helper(sim)
