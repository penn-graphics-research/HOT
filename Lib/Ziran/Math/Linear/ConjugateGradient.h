...
public:
    ...
    int solve(const TM& A, TV& x, const TV& b, const bool verbose = false)
    {
        ...
        T forcing_sequence = Base::relative_tolerance;
        T local_tolerance = forcing_sequence * residual_preconditioned_norm;
        ZIRAN_INFO("Inexact CG forcing sequence: residual_preconditioned_norm = ", residual_preconditioned_norm, ", forcing sequence = ", forcing_sequence);

        for (cnt = 0; cnt < Base::max_iterations; ++cnt) {
            if (residual_preconditioned_norm < local_tolerance) {
                ZIRAN_VERB_IF(verbose, "\tCG terminates at ", cnt, "; (preconditioned norm) residual = ", residual_preconditioned_norm, " tol: ", local_tolerance, " with forcing sequence ", forcing_sequence);
                return cnt;
            }

            if (cnt % 50 == 0) {
                ZIRAN_VERB_IF(verbose, "\tCG iter ", cnt, "; (preconditioned norm) residual = ", residual_preconditioned_norm, " tol: ", local_tolerance, " with forcing sequence ", forcing_sequence);
            }
