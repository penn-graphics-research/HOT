...
public:
    ...
    MpmParticleHandleBase<T, dim> sampleFromVdbFileFullyRandom(std::string filename, T density, T particles_per_cell = (1 << dim))
    {
        T per_particle_volume = std::pow(mpm.dx, dim) / particles_per_cell;
        VdbLevelSet<float, dim> vdbls(filename); // houdini's vdb files are stored with floats
        Vector<float, dim> min_corner, max_corner;
        ...
        PoissonDisk<T, dim> pd(/*random seed*/ 123, 0, min_corner.template cast<T>(), max_corner.template cast<T>());
        pd.setDistanceByParticlesPerCell(mpm.dx, particles_per_cell);
        StdVector<TV> old_samples;
        if (dim == 3)
            pd.sampleFromPeriodicData(old_samples, [&](const TV& x) { return vdbls.inside(x.template cast<float>()); });
        else
            pd.sample(old_samples, [&](const TV& x) { return vdbls.inside(x.template cast<float>()); });
        size_t N = old_samples.size();

        RandomNumber<float> rnd(123);
        StdVector<TV> samples;
        while (samples.size() != N) {
            TV xx = rnd.randInBox(min_corner, max_corner).template cast<T>();
            if (vdbls.inside(xx.template cast<float>()))
                samples.push_back(xx);
        }
    ...
    MpmParticleHandleBase<T, dim> sampleInAnalyticLevelSetFullyRandom(AnalyticLevelSet<T, dim>& levelset, T density, T particles_per_cell)
    {
        StdVector<TV> samples0;
        T total_volume = sampleInAnalyticLevelSetHelper(levelset, particles_per_cell, samples0);
        size_t N = samples0.size();
        TV min_corner, max_corner;
        levelset.getBounds(min_corner, max_corner);
        ...
        StdVector<TV> samples;
        while (samples.size() != N) {
            TV xx = rnd.randInBox(min_corner, max_corner);
            if (levelset.inside(xx))
                samples.push_back(xx);
        }
