...
struct RotationExtractor;

template <typename T>
struct RotationExtractor<T, 3> {
    using TM = Eigen::Matrix<T, 3, 3>;
    using TV = Eigen::Matrix<T, 3, 1>;
    static TM rotate(const TV& a)
    { ///< rotate to (1, 0, 0)
        static TV unitNorm = TV::Zero();
        unitNorm(0) = 1;
        TM R = static_cast<TM>(Eigen::Quaternion<T>().setFromTwoVectors(a, unitNorm));
        return R;
    }
};

template <typename T>
struct RotationExtractor<T, 2> {
    using TM = Eigen::Matrix<T, 2, 2>;
    using TV = Eigen::Matrix<T, 2, 1>;
    static TM rotate(const TV& a)
    { ///< rotate to (1, 0)
        T sqr = a(0) * a(0) + a(1) * a(1);
        T c = a(0) / sqr;
        T s = a(1) / sqr;
        TM R;
        R(0, 0) = R(1, 1) = c;
